...
// Deposits into backend/my-node-server/uploads
  ...
  try {
    ...
    if (user.mfaEnabled) {
      if (!otp) {
        return res
          .status(400)
          .json({ success: false, error: 'OTP required' })
      }
...
export const getCurrentUser = async (req, res) => {
  try {
    const user = await User.findById(req.user.id)
      ...
      latitude: 30.005095, // Example: GIU latitude
      longitude: 31.700394, // Example: GIU longitude
    };
...
export const updateCurrentUser = async (req, res) => {
  try {
    ...
    // only parse if it's a JSON‐encoded string
    if (pickupLocation) {
    ...
    // dropoff is always GIU—ignore whatever the client sends:
    ...
    // file upload handling stays the same…
    if (req.file) {
