...
export default function ViewBusDetails() {
  ...
  useEffect(() => {
    fetchWithAdminAuth('/admin/users?role=student')
      .then(res => res.json())
      .then(data => setStudentsList(data.users || []))
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <Card className="mt-6">
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>
                {bus.studentsAssigned?.length || 0} students currently assigned
              </CardDescription>
            </div>
            <Button
              onClick={() => router.push(`/admin/buses/${bus._id}/assign-students`)}
              className="gap-2"
            >
        ...
        <CardContent>
          {bus.studentsAssigned && bus.studentsAssigned.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {bus.studentsAssigned.map((student) => (
                <div
                  key={student._id}
            ...
            <p className="text-muted-foreground">No students assigned to this bus</p>
          )}
        </CardContent>
      </Card>
